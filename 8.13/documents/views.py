...
class MyTeamViewset(mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        #找出我的团队
        self.queryset = Document.objects.filter(Q(id__in=ids)|Q(create_user=request.user), Q(type=1))
...
class MyDocViewset(mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        for team in teams:
            ids.append(team.document_id)
        # 找出我的文档
        self.queryset = Document.objects.filter(Q(id__in=ids)|Q(create_user=request.user), Q(type=0),Q(parent_doc=None))
...
class ChildDocViewset(mixins.RetrieveModelMixin, viewsets.GenericViewSet):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if instance.type == 1:
            ...
            aws = []
            for child in childs:
                d = {'id': child.id, 'name': child.name, 'create_user': child.create_user_id, 'content': child.content,
                     ...
                     'last_modify_user': child.last_modify_user_id}
                aws.append(d)
            return JsonResponse(aws, safe=False)
        else:
            return Response(status=status.HTTP_401_UNAUTHORIZED)
